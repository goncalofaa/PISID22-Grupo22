        ...
        while (true) {
    		try {
    			...
    			if(message != null && !message.isEmpty()){
	    			...
	    			st = msConnection.getConnectionSQL().prepareStatement("SELECT * FROM parametrozona");
	    			rs = st.executeQuery();
	    			while(rs.next()) {
	    			...
	    			st = msConnection.getConnectionSQL().prepareStatement("SELECT * FROM cultura");
	    			rs = st.executeQuery();
	    			while(rs.next()) {
	    				...
	    				if(idZonaCultura == sensorZona) {
	    					st = msConnection.getConnectionSQL().prepareStatement("SELECT * FROM parametrocultura WHERE IDParametroCultura = ?"); // não sei se a comparação funciona com string
